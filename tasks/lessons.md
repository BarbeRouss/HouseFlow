...
### Après un redémarrage de conteneur, `dockerd` garde l'ancien port du proxy sortant
**Contexte:** Même session que ci-dessus, après un redémarrage de conteneur survenu en cours de tâche (#230). Le même sous-ensemble de tests, qui venait de passer en série, s'est remis à échouer massivement (171/173) — cette fois avec le run qui restait bloqué plus de 3 minutes sans sortie, pas seulement lent.
**Cause:** Le proxy sortant de la session (`$HTTPS_PROXY`) change de port à chaque redémarrage de conteneur (ici 42989 → 32877), mais `dockerd`, lancé avant le redémarrage, avait figé l'ancien port dans son environnement au démarrage. `docker info` montrait encore `HTTPS Proxy: http://127.0.0.1:42989` — mort — si bien que tout `docker pull` (dont l'image `postgres` que Testcontainers/Aspire tire à chaque run d'intégration) échouait en `connection refused`, et chaque test qui en dépendait épuisait le timeout HTTP de l'API plutôt que de démarrer.
**Leçon:** Après toute notification de redémarrage de conteneur, avant de relancer des tests qui dépendent de Docker, comparer `docker info | grep -i "https proxy"` à `echo $HTTPS_PROXY` du shell courant. En cas de désaccord, `kill` le PID de `dockerd` (`pgrep -x dockerd`) et le relancer (`nohup dockerd &`) : il hérite alors du proxy à jour. Un échec massif et *bloquant* (pas seulement des timeouts, mais un run qui ne produit plus aucune sortie) après un redémarrage de conteneur est le signal à chercher en premier — avant de re-soupçonner une contention CPU comme la leçon précédente.

### Un volume Docker nommé survit à un changement de version d'image Postgres
**Contexte:** Même session, même tâche (#230). Après avoir corrigé le proxy de `dockerd` (leçon précédente), les tests d'intégration échouaient encore en `TaskCanceledException` à 100 s — mais cette fois avec Aspire qui loggait `Resource 'postgres' has entered the 'Exited' state prematurely`, le conteneur `postgres` refusant de démarrer avec l'erreur `these Docker images are configured to store database data in a format which is compatible with "pg_ctlcluster"... there appears to be PostgreSQL data in: /var/lib/postgresql` provenant d'une version majeure antérieure.
**Cause:** Aspire réutilise un volume Docker **nommé** (pas anonyme) entre deux runs du même AppHost, pour ne pas réinitialiser la base à chaque test — `docker volume ls` montrait `houseflow.apphost-<hash>-postgres-data`. Ce volume contenait encore des données au format PostgreSQL 17 (image utilisée avant que la dépendance Aspire ne soit montée en 18.3, cf. #159 mergé sur `main` pendant cette même tâche). L'image 18 refuse de démarrer sur un volume d'une version majeure différente plutôt que de corrompre silencieusement les données.
**Leçon:** Un conteneur applicatif qui `Exited` immédiatement après sa création — sans jamais atteindre `Running` — n'est presque jamais un problème de CPU ou de réseau : lire les logs du conteneur lui-même (ici via la sortie `-v detailed` d'Aspire, colonne `fail:`), pas seulement l'exception HTTP côté appelant qui n'est qu'un symptôme du timeout. `docker volume ls` pour repérer un volume nommé par l'AppHost, `docker volume rm` pour le vider — sans risque, un environnement de test recrée son schéma et ses données au prochain run.

### Un dev-server Blazor longue-durée sert des assemblies WASM périmées après plusieurs rebuilds
**Contexte:** Même session, même tâche (#230). Après plusieurs redémarrages de conteneur et rebuilds successifs (`dotnet build src/HouseFlow.Web`), `scripts/verify-e2e.sh` échouait uniquement sur `device-management.spec.ts` et `device-details.spec.ts` (9/9 tests, systématiquement, même isolés en un seul worker) : timeout sur `getByPlaceholder('Jean')` de la page d'inscription. `frontend_assets_ok` du script (probe HTTP des `_framework/*.js`) ne détectait rien d'anormal — "Frontend already running on :3000".
**Cause:** Le processus `dotnet run` du frontend (`blazor-devserver`), démarré une fois le Sep23 et jamais redémarré à travers plusieurs `dotnet build` ultérieurs, servait un mélange d'assemblies WASM d'âges différents. La page d'inscription chargeait alors avec une erreur JS non catchée (`ManagedError: Could not resolve type with token ... in assembly 'BlazorBlueprint.Primitives'`), visible uniquement dans la console navigateur (`page.on('console')`/`pageerror`) ou dans le "page snapshot" du rapport Playwright (« An unhandled error has occurred » — l'error boundary de Blazor), jamais dans les logs du script ni dans l'exception Playwright elle-même, qui n'est qu'un symptôme (timeout d'attente d'un élément qui ne rendra jamais). Le process API (`dotnet run --project src/HouseFlow.API`), lui aussi resté vivant depuis Sep23 à travers plusieurs merges de code, tournait pareillement sur un binaire obsolète.
**Leçon:** La détection de staleness de `verify-e2e.sh` (probe HTTP) ne couvre pas une incohérence interne entre assemblies WASM déjà chargées — seul un rechargement complet du process le peut. Après un rebuild qui suit un ou plusieurs redémarrages de conteneur, ne pas se fier à "Backend/Frontend already running" : `pkill -9 -f blazor-devserver`, `pkill -9` sur les binaires compilés `HouseFlow.API`/`HouseFlow.Web` (pas seulement le wrapper `dotnet run`, qui a son propre process enfant), puis laisser le script les redémarrer proprement. Pour diagnostiquer un échec Playwright localisé à quelques specs et systématique même isolé : lire le "page snapshot" du rapport (texte visible à l'écran au moment du timeout) avant de soupçonner une contention de ressources — un texte d'erreur applicative y est souvent explicite là où la stack Playwright ne l'est pas.
...
### Un groupe de concurrence GitHub n'est pas une file d'attente, sauf à le demander (`queue: max`)
**Contexte:** Les previews « cancelled » de #233, #235, #237 et #163 : jobs sans runner (`runner_id=0`), annulés à la seconde où une autre preview entrait dans le groupe `ovh-dns-zone`. #238 a raccourci le verrou (~25 min → quelques secondes), rendant les annulations rares mais pas impossibles ; #252 a ensuite vérifié la doc GitHub Actions plutôt que de supposer, et trouvé `queue: max`.
**Cause:** Par défaut (`queue: single`), un groupe garde au plus **un** job en cours et **un** en attente ; un nouveau venu annule celui qui attend. `cancel-in-progress: false` ne protège que le job en cours, pas celui qui patiente.
**Leçon:** Avant d'affirmer qu'une limitation d'un outil est définitive, vérifier sa documentation à jour — GitHub Actions propose `queue: max` (jusqu'à 100 jobs en attente, exécutés dans l'ordre, sans annulation) depuis un moment ; ce n'est ni un défaut ni une absence de solution. Seule contrainte : `queue: max` est incompatible avec `cancel-in-progress: true`. Un verrou `concurrency` doit quand même rester le plus court possible — la file n'aide pas si l'attente devient déraisonnable. Pour diagnostiquer un « cancelled », comparer l'heure d'annulation avec l'heure de création des runs du même groupe (`gh api …/runs/<id>/jobs`) avant de soupçonner une annulation manuelle ou un quota.
