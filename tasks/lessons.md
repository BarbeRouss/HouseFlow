...
### Le défaut d'un groupe de concurrence GitHub n'est pas une file d'attente — mais une vraie file existe
**Contexte:** Les previews « cancelled » de #233, #235, #237 et #163 : jobs sans runner (`runner_id=0`), annulés à la seconde où une autre preview entrait dans le groupe `ovh-dns-zone`. #238 avait réduit la durée du verrou (~29 s) sans changer son mode ; #252 a éliminé la cause plutôt que d'en réduire la fréquence.
**Cause:** Sans le préciser, un groupe de concurrence utilise `queue: single` : au plus **un** job en cours et **un** en attente ; un nouveau venu annule celui qui attend. `cancel-in-progress: false` ne protège que le job en cours. J'ai affirmé qu'aucune vraie file n'était possible sans vérifier la documentation de l'outil — GitHub Actions propose `queue: max` (jusqu'à 100 jobs en attente, exécutés dans l'ordre, sans annulation ; incompatible avec `cancel-in-progress: true`).
**Leçon:** Avant d'écrire qu'une option n'existe pas ou qu'un comportement est subi, vérifier la doc de l'outil plutôt que de se fier à un premier essai ou à une impression. Un verrou `concurrency` doit rester court quel que soit le mode de file (la durée détermine le débit), mais `queue: max` élimine l'annulation elle-même là où réduire la durée ne fait que la rendre rare. Pour diagnostiquer un « cancelled », comparer l'heure d'annulation avec l'heure de création des runs du même groupe (`gh api …/runs/<id>/jobs`) avant de soupçonner une annulation manuelle ou un quota.
