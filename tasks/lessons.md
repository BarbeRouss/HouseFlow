...
**Cause:** Le devcontainer est le chemin du dev local. En session web, `init-session.sh` installe tout sur l'hôte (.NET dans `/usr/share/dotnet`, Postgres 16 local) : c'est là que la checklist doit tourner. En plus, `verify-e2e.sh 2>&1 | tail` ne rend jamais la main : l'API et le front démarrés via `setsid` héritent du pipe et le gardent ouvert.
**Leçon:** En session web : `export PATH=/usr/share/dotnet:$PATH POSTGRES_HOST=postgres`, alias `127.0.0.1 postgres` dans `/etc/hosts` (le fixture d'intégration exige ce nom), Postgres 16 démarré (`pg_ctlcluster 16 main start`), et `bash scripts/verify-e2e.sh > /tmp/e2e.log 2>&1` — jamais piper sa sortie. Si Docker Hub répond 429, Aspire ne peut pas tirer son image Postgres : `POSTGRES_HOST=postgres` sur le Postgres local contourne proprement.
