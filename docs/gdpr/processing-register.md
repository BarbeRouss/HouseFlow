...
| **Transferts hors UE** (e) | **Aucun transfert volontaire.** Hébergement Azure **West Europe** (Pays-Bas). Les accès support Microsoft depuis un pays tiers sont couverts par le DPA Microsoft (CCT du 04/06/2021 + certification EU-US Data Privacy Framework du 10/07/2023). |
...
| **Destinataires internes** (d) | Les membres de la maison concernée, selon leur rôle : `Owner`, `CollaboratorRW`, `CollaboratorRO`, `Tenant`. Le champ `Cost` n'est visible que des membres dont `CanViewCosts` est vrai (faux par défaut pour les locataires). |
...
| **Mécanisme technique de purge** | Job Hangfire récurrent `DataRetentionJob`, exécuté **quotidiennement à 03:00 UTC**, paramétré par la section `DataRetention` de `appsettings.json`. Il applique, dans l'ordre : troncature des IP de plus de 30 jours (`IpAddressAnonymizer`), suppression des refresh tokens et clés API révoqués ou expirés depuis plus de 30 jours, anonymisation des entrées d'audit de plus d'un an, suppression des entrées d'audit de plus de trois ans. Chaque exécution est journalisée avec les volumes traités (preuve d'accountability, Art. 5(2)). |
...
| **Sous-traitants** | Microsoft Azure (opérations réalisées en ligne). Le journal des demandes est tenu dans le dépôt Git du projet. |
| **Transferts hors UE** (e) | Aucun. |
...
### Traitement n° 7 — Environnements techniques (préproduction et prévisualisations)

| Rubrique (Art. 30(1)) | Contenu |
|---|---|
| **Référence** | TR-07 |
| **Finalité(s)** (b) | Tester, recetter et valider les évolutions du service avant leur mise en production. |
| **Base légale** | **Art. 6(1)(f) — intérêt légitime** à disposer d'un environnement de recette représentatif, **strictement subordonné** à l'absence de données personnelles réelles. |
| **Catégories de personnes** (c) | **Aucune personne réelle.** Les environnements ne contiennent que des données fictives ou anonymisées. |
| **Catégories de données** (c) | **Préproduction** : copie de la structure de production dont toutes les données à caractère personnel ont été **écrasées** par `scripts/sanitize-pii.sh` — emails remplacés par `user<uuid>@fake.local`, prénoms et noms par `Prénom_<8 car.>` / `Nom_<8 car.>`, `PasswordHash` remplacé par une valeur inerte, jetons de rafraîchissement et d'invitation remplacés, IP remplacées par `0.0.0.0`, `UserAgent` remplacé, `OldValues` / `NewValues` / `ChangedProperties` vidés. **Prévisualisations de PR** : base neuve et vide à chaque déploiement, alimentée par un unique **utilisateur de démonstration** (`demo@demo.com`, identité fictive) créé lorsque `DEMO_MODE=true`. **Développement local** : utilisateur `admin@admin.com` fictif. |
| **Source des données** | Copie de production **systématiquement anonymisée** avant toute exposition ; jeux de données fictifs. |
...
| **Transferts hors UE** (e) | GitHub Actions et GitHub Container Registry peuvent s'exécuter hors EEE. **Aucune donnée personnelle d'utilisateur n'y transite** : seuls le code source, les artefacts de compilation et les secrets de déploiement (jamais de données applicatives) y sont traités. |
| **Durée de conservation** (f) | Préproduction : durée de la campagne de recette, base réinitialisée à chaque synchronisation. Prévisualisations de PR : détruites à la fermeture de la pull request. |
| **Mécanisme technique** | `scripts/sanitize-pii.sh` — refuse explicitement de s'exécuter sur une base nommée `houseflow` (production) et vérifie, en fin d'exécution, qu'aucune adresse email non anonymisée ne subsiste. `.github/workflows/pr-preview.yml` pour les environnements éphémères. |
| **Règle de gouvernance** | **Interdiction absolue de restaurer une copie de production non anonymisée dans un environnement non productif.** L'exécution de `sanitize-pii.sh` est une étape obligatoire et non contournable de toute synchronisation production → préproduction. |
| **Mesures de sécurité** (g) | Bases distinctes et cloisonnées ; secrets propres à chaque environnement ; aucun secret de production hors du coffre ; authentification OIDC sans secret de longue durée entre GitHub Actions et Azure. |
| **Référence code** | `scripts/sanitize-pii.sh`, `.github/workflows/pr-preview.yml`, `src/HouseFlow.API/Program.cs` (bloc `DEMO_MODE`). |
...
|---|---|---|---|
| 1 | Compléter l'**adresse postale** du responsable de traitement | Éditeur | avant publication de la politique |
| 2 | Arrêter l'**autorité de contrôle chef de file** (CNIL ou APD) selon l'établissement principal | Éditeur | — |
| 3 | Archiver le **DPA Microsoft** (date et version) et consigner la référence dans [subprocessors.md](./subprocessors.md) | Référent vie privée | — |
