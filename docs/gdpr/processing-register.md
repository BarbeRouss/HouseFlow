...
| **Mécanisme technique** | `scripts/sanitize-pii.sh` — refuse explicitement de s'exécuter sur une base nommée `houseflow` (production) et vérifie, en fin d'exécution, qu'aucune adresse email, adresse postale ni note d'entretien non anonymisée ne subsiste. `.github/workflows/pr-preview.yml` pour les environnements éphémères. |
