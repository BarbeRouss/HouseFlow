...
# rôle de l'infrastructure ». Chacune a son homologue du même nom dans la souscription des
...
# Attachée au job `dbtools dump` de la prod, qui publie le dump pseudonymisé. Son homologue
# jetable ne fait que le lire.
resource "azurerm_user_assigned_identity" "dumps" {
  name                = "id-houseflow-dumps"
  location            = var.location
  resource_group_name = data.azurerm_resource_group.shared.name
}
